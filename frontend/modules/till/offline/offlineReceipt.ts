...
export function buildOfflineReceipt(input: OfflineReceiptInput): Sale {
  ...
  const saleLines = lines.map((line, i) => {
    ...
    return {
      ...
      promotionDiscountCents: line.promotionCents ?? 0,
      promotion: line.promotionName ? { id: 0, name: line.promotionName } : null,
  ...
  return {
    ...
    discountCents: lines.reduce((s, l) => s + l.discountCents + (l.promotionCents ?? 0), 0),
    totalCents,
