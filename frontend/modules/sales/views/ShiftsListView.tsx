              ...
              <Table.Tbody>
                {data?.items.map((shift) => (
                  <Table.Tr key={shift.id}>
                    ...
                    <Table.Td>
                      <Text size="sm">
                        {dayjs(shift.openedAt).format("DD MMM, h:mm a")}
                      </Text>
                      {shift.isOpen ? (
                        ...
                        <Text size="xs" c="dimmed">
                          ...
                          {dayjs(shift.closedAt).format(
                            dayjs(shift.closedAt).isSame(shift.openedAt, "day")
                              ? "h:mm a"
                              : "DD MMM, h:mm a",
                          )}
                        </Text>
                    ...
                    <Table.Td ta="right">
                      {formatKes(shift.takings.cashCents)}
                    </Table.Td>
                    <Table.Td ta="right">
                      {formatKes(shift.takings.mpesaCents)}
                    </Table.Td>
                    <Table.Td ta="right">
                      {formatKes(shift.takings.cardCents)}
                    </Table.Td>
                    <Table.Td ta="right">
                      {shift.isOpen ? "—" : formatKes(shift.expectedCashCents)}
                    </Table.Td>
                    <Table.Td ta="right">
                      {shift.isOpen ? "—" : formatKes(shift.countedCashCents)}
                    </Table.Td>
                    <Table.Td ta="right">
                      {shift.isOpen ? (
                        "—"
                        ...
                        <Tooltip
                          label={shift.closeNote ?? "No note"}
                          disabled={!shift.closeNote}
                          withArrow
                        >
                          <Text
                            size="sm"
                            fw={700}
                            c={shift.varianceCents === 0 ? "green.7" : "red.7"}
                          >
