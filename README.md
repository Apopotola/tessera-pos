...
| Dashboard | Owner KPIs from the requirements: net sales vs the same weekday last week, takings by tender, gross profit and margin, discounts/voids/refunds by cashier, shrinkage as % of cost of goods, cash variance by cashier (30 days), low stock with fast movers first, best sellers and slow movers (no sale in 60 days), stock value, eTIMS pending/failed with the oldest wait, branch comparison; plus stock, purchasing and cash-up alerts | Dashboard (each KPI opens its report; tiles per role in Settings) |
...
| Credit & supplier payments (Phase 2) | Customer credit accounts (limit, terms, on hold at 0), sales "on account" at the till with manager approval over the limit, returns refunded to the account, payments received and reversed, statements and aging; supplier balances from invoices − payments − credit notes, partial payments and reversals, invoices on query, aging; receivables/payables aging reports; append-only payment tables | Customers → Customer accounts, customer credit section; Purchasing → Supplier accounts; till "On account" payment |
| Notifications (Phase 2) | Alerts raised from sales (item crosses its reorder level), large refunds, cash-ups outside the allowed variance, KRA-refused invoices, eTIMS waiting too long, nightly low-stock digest and the end-of-day summary; in-app inbox per user by permission and branch; SMS / WhatsApp / email outbox for owners (SMS and WhatsApp demo-logged until a provider is chosen) | Header bell; Administration → Messages sent |
| Promotions (Phase 2) | Owner-approved price rules (maker–checker; an owner's own is approved at once): % or KES off, minimum quantity across matching items (mix and match), categories / brands / items, bottles or tots, dates, weekdays, happy-hour window, branches; best promotion per line applied by the API and mirrored at the till (offline too); recorded on the sale line and sent to eTIMS as discount | Catalogue → Promotions; till shows promotions per line; receipts |
...
## Credit accounts and supplier payments (Phase 2)

- **Switch on** *Settings → Payments → Customer credit*, then open an account on the customer's page (*Credit account → Open credit account*: limit and payment terms; permission `customers.credit`, Owner/Admin/Accountant). A limit of 0 puts the account on hold.
- **Till:** pick the account customer, then **On account** (full or split). Over the limit a manager approves with their PIN — or every time, per *Credit sale approval*. Offline tills never take credit. A return of a sale on account goes back to the account first, the rest in cash.
- **Balances are never stored:** a customer owes sales on account − returns to the account − payments; we owe a supplier invoices − payments − credit notes. Payments settle the oldest amounts first for the aging (0–30, 31–60, 61–90, 90+ days). Payments are append-only (DB trigger); a mistake is corrected with **Reverse**, which adds a reversal row.
- **Customer payments** (`customers.payments`: managers, accountant) are recorded at a branch and numbered `BRANCH-RCP-000001`. **Supplier payments** (`purchasing.pay`: owner, admin, accountant) are numbered `SP-000001`. A supplier's credit note for returned stock is recorded with its amount on the return (*Returns to supplier → Record*), once.
- Supplier invoices that do not match the goods received are shown **on query**: still owed, but check with the supplier before paying.
- Reports: *Receivables aging* and *Payables aging* (financial, as-at date, CSV).

## Notifications (Phase 2)

- **Alerts** (switch each on or off in *Settings → Notifications*): low stock (when a sale takes an item to or below its reorder level, plus a nightly digest at 06:00), large refund (from *A refund is "large" from*, default KES 5,000), cash variance (a cash-up outside the allowed variance), eTIMS failure (KRA refused an invoice, or invoices waiting over an hour), and the owner's end-of-day summary at *Daily report time* (default 21:00).
- **Who gets them:** the bell in the header shows each alert to everyone who can act on it at that branch (e.g. managers and storekeepers for low stock); owners get everything. The bell's polling does not keep an idle session alive.
- **SMS / WhatsApp / email** go to owners (phone and email from *Users & roles*), per *Channels*. Email uses Laravel mail (`MAIL_MAILER`, `log` in development). SMS and WhatsApp use the `log` driver (`NOTIFY_SMS_DRIVER`, `NOTIFY_WHATSAPP_DRIVER`): messages are written to the server log, not delivered, until a provider (e.g. Africa's Talking) and a registered sender ID are set up. *Administration → Messages sent* lists every message and marks demo ones.
- The scheduler runs `notifications:run` every minute (daily summary, eTIMS waiting, sending the outbox) and `notifications:low-stock` nightly — keep `php artisan schedule:work` running in development. `php artisan notifications:run --summary-now` sends today's summary straight away.

## Promotions (Phase 2)

- **Catalogue → Promotions → New promotion**: e.g. *Buy 6 wine, 10% off* (category Wine, minimum 6 — any mix of wines counts) or a *happy hour* (tots, KES 50 off each, 17:00–19:00, Friday and Saturday). A manager's promotion waits for the owner's approval (`catalogue.promotions.approve`, owners only); the owner's own is approved as it is saved. An approved promotion is never edited: **End now** and set up a new one.
- **At the till** the best promotion per line is applied automatically and shown under the line and on the receipt; a cashier's discount can come on top within their limit. Lines at a changed price or at the wholesale price get none. The API re-prices every sale with the same rules (`Modules\Catalogue\Support\PromotionEngine`, mirrored in `frontend/modules/till/promotions.ts`), stores the promotion and its discount on the sale line, and sends promotion + discount to eTIMS as the line discount.
- **Loyalty points** are not built: the requirements list them as future/optional, needing customer consent and a check that Kenyan alcohol-promotion rules allow them (REQUIRES VALIDATION).
