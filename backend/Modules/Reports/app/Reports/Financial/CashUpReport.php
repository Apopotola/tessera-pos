...
class CashUpReport extends Report
{
    ...
    public function run(ReportFilters $filters, User $user): ReportResult
    {
        $rows = DB::table('shifts as sh')
            ...
            ->select('sh.closed_at', 't.name as till', 'br.name as branch', 'sh.user_id', 'sh.opening_float_cents', 'sh.expected_cash_cents', 'sh.counted_cash_cents', 'sh.variance_cents', 'sh.close_note')
            ->get();
