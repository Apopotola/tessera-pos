...
final class SettingsRegistry
{
    ...
    public static function all(): array
    {
        return [
            ...
            'staff.password_expiry_days' => self::f('staff', 'Password expiry (days)', 'number', 0, 'O', min: 0, max: 365, help: '0 = never.',
                available: false, note: 'Arrives with two-step login.'),
            'staff.two_step_login' => self::f('staff', 'Two-step login for owners and admins', 'boolean', true, 'O', available: false, note: 'Arrives with two-step login.'),
            ...
            'notifications.recipients' => self::f('notifications', 'Alert recipients', 'multiselect', ['low_stock', 'large_refund', 'cash_variance', 'etims_failure', 'daily_summary'], 'O',
                ...
                available: false, note: 'Alerts go to the owner on the dashboard today; SMS, WhatsApp and email arrive with notifications.'),
            'notifications.channels' => self::f('notifications', 'Channels', 'multiselect', ['in_app', 'sms'], 'O',
                options: ['in_app' => 'In-app', 'sms' => 'SMS', 'whatsapp' => 'WhatsApp', 'email' => 'Email'], available: false, note: 'Arrives with notifications.'),
            'notifications.sms_sender' => self::f('notifications', 'SMS sender name', 'text', 'TESSERA', 'T', max: 11, available: false, note: 'Needs a registered sender ID.'),
            'notifications.customer_sms_templates' => self::f('notifications', 'Customer SMS wording', 'lines', null, 'O', max: 3, available: false, note: 'Arrives with notifications.'),
            'notifications.daily_report_time' => self::f('notifications', 'Daily report time', 'time', '21:00', 'O', available: false, note: 'Arrives with notifications.'),
